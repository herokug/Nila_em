...
×───ꜱᴇʟᴇᴄᴛ ꜰɪʟᴇ ᴛʏᴘᴇ───×`,
  ...
  async (m, conn) => {
    ...
    /// console.log(m.text)
    //f(!m.text) return await conn.sendMessage( m.from, { text: config.reply.owner }, { quoted: m } );

    try {

        let media = await yta (m.text, '128kbps')
        let media2 = Math.floor((2.5 * media.filesize) / 1000)

                let buttons = [
                        {buttonId: `.ytmp3zl ${m.text} 320kbps`, buttonText: {displayText: `ʜɪɢʜ [${media2} MB]`}, type: 1},
                        {buttonId: `.ytmp3zl ${m.text} 128kbps`, buttonText: {displayText: `ʟᴏᴡ [${media.filesizeF}]`}, type: 1},
...
◂◂◂◂◂◂◂◂◂◂◂◂◂◂◂◂◂◂◂◂◂◂◂◂◂◂◂◂◂◂◂`,
  ...
  async (m, conn) => {
    ...
    let media = await ytv(m.text, '360p')
    let media2 = await ytv(m.text, '720p')
    let media3 = await ytv(m.text, '1080p')

                let buttons = [
                    {buttonId: `.ytmp4zl ${m.text} 360p`, buttonText: {displayText: `360ᴘ [${media.filesizeF}]`}, type: 1},
                    {buttonId: `.ytmp4zl ${m.text} 720p`, buttonText: {displayText: `720ᴘ [${media2.filesizeF}]`}, type: 1},
                    
                    {buttonId: `.ytmp4zl ${m.text} 1080p`, buttonText: {displayText: `1080ᴘ [${media3.filesizeF}]`} , type: 1},
...
◂◂◂◂◂◂◂◂◂◂◂◂◂◂◂◂◂◂◂◂◂◂◂◂◂◂◂◂◂◂◂`,
    ...
    nila.addCommand({ pattern: ['doctsl'], usage: 'downloadaer', sucReact: `${emojis[Math.floor(Math.random() * emojis.length)]}` , category: ["search", "all"] },
  async (m, conn) => {
    ...
    let buttons = [
                         {buttonId: `.docytsongzl ${m.text}`, buttonText: {displayText: `ᴅᴏᴄ ᴀᴜᴅɪᴏ`}, type: 1},
                         {buttonId: `.docytvideoszl ${m.text}`, buttonText: {displayText: `ᴅᴏᴄ ᴠɪᴅᴇᴏ`}, type: 1},
