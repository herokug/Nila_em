...
//------------------team and team mem-----------------------
    ...
    const chatbot = (await axios.get(`http://api.brainshop.ai/get?bid=170077&key=mnLPKvyUSMLb58RE&uid=${m.sender}&msg=${m.displayText}`)).data
    ...
    try {
     ...
     const chatbot2 = (await axios.get(`http://api.brainshop.ai/get?bid=170077&key=mnLPKvyUSMLb58RE&uid=${m.sender}&msg=${tanlate2.responseData.translatedText}`)).data
     
     const tanlates = (await axios.get(`https://api.mymemory.translated.net/get?q=${chatbot2.cnt} &langpair=en-US|si-LK`)).data

    console.log(chatbot.cnt)

    await conn.sendMessage(m.chat, {text: `${tanlates.responseData.translatedText}` } , { quoted : m });
